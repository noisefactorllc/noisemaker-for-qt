...
- Next action: Find a Windows golden path on the candidate's rasterizer. CI run 36038944290 (8fce5e8), Linux render-smoke: goldens drawn by "ANGLE (Mesa, llvmpipe (LLVM 20.1.2 256 bits), OpenGL 4.5)" grade all 6 smoke fixtures at max 0, meshRenderCustom included. Windows render-smoke-windows: with Mesa registered as the MSOGL ICD, Chromium reported "golden WebGL renderer: unavailable", and ANGLE failed ERR_SHADER_COMPILE on a trivial ES 3.0 vertex shader. The runner's opengl32 did not load the Mesa ICD. Before that, a Linux arm64 Docker run of the committed Linux steps graded all 6 smoke fixtures at max 0 with goldens drawn by "ANGLE (Mesa, llvmpipe (LLVM 20.1.2 128 bits), OpenGL 4.5)"; the former max-1 fixtures also went to 0. Do not change tolerances or drop the fixture.
- Dependencies: None.
