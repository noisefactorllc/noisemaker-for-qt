...
- Function values: no runtime evaluates an arrow-function or bare state-value parameter (`noise(octaves: () => time * 4)`, `noise(ridges: time)`), the reference included. `render()` binds what the reference demo shows after it loads the program: the spec default clamped to `min`/`max` for a number, `true` for a boolean, the default (else zeros) for a vector or color, also in the parameter's `_node_N`/`_chain_N` copies. A parameter the demo shows no control for (`ui.control: false` or `ui.hidden: true` in the effect JSON) keeps the compiled object, which binds as 0. `applyStepParameterValues` and `setUniform` replace a function value.
