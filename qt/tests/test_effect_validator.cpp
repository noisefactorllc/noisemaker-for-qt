...
int main() {
    ...
    // --- differential corpus: replay the reference's exact error strings ---
    // parity/effect-validator-corpus.json is generated by
    // parity/gen-effect-validator-corpus.mjs against the pinned reference
    // commit; each probe's referenceErrors is the reference
    // validateEffectDefinition() output, verbatim. The port must reproduce
    // the same error list per probe. Comparison is over SORTED lists: the
    // reference emits Object.entries insertion order, the port emits
    // QJsonObject's sorted key order (documented deviation in
    // effect_validator.h); the error SET must be byte-identical.
    {
        QFile corpusFile(QStringLiteral("parity/effect-validator-corpus.json"));
        if (!corpusFile.open(QIODevice::ReadOnly)) {
            std::printf("FAIL: differential corpus missing: parity/effect-validator-corpus.json\n");
            ++g_failures;
        } else {
            const QJsonDocument corpusDoc = QJsonDocument::fromJson(corpusFile.readAll());
            const QJsonArray probes = corpusDoc.object().value(QStringLiteral("probes")).toArray();
            const QString commit = corpusDoc.object().value(QStringLiteral("provenance"))
                                       .toObject().value(QStringLiteral("referenceCommit")).toString();
            if (probes.isEmpty() || commit.isEmpty()) {
                std::printf("FAIL: differential corpus has no probes/provenance\n");
                ++g_failures;
            } else {
                int mismatches = 0;
                for (const QJsonValue& probeValue : probes) {
                    const QJsonObject probe = probeValue.toObject();
                    const QJsonObject def = probe.value(QStringLiteral("def")).toObject();
                    QStringList wantSorted;
                    for (const QJsonValue& e :
                         probe.value(QStringLiteral("referenceErrors")).toArray()) {
                        wantSorted << e.toString();
                    }
                    wantSorted.sort();
                    const std::vector<std::string> got = validateEffectDefinition(def);
                    QStringList gotSorted;
                    for (const std::string& e : got) gotSorted << QString::fromStdString(e);
                    gotSorted.sort();
                    if (wantSorted != gotSorted) {
                        ++mismatches;
                        std::printf("FAIL: corpus probe '%s':\n  want: %s\n  got:  %s\n",
                                    probe.value(QStringLiteral("id")).toString().toUtf8().constData(),
                                    wantSorted.join(QStringLiteral(" | ")).toUtf8().constData(),
                                    gotSorted.join(QStringLiteral(" | ")).toUtf8().constData());
                    }
                }
                std::printf("[gap-003 differential] probes=%d mismatch=%d reference=%s\n",
                            int(probes.size()), mismatches, commit.toUtf8().constData());
                if (mismatches != 0) {
                    std::printf("FAIL: differential corpus\n");
                    ++g_failures;
                }
            }
        }
